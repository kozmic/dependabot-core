...
#
# Usage:
#   ruby bin/dry-run.rb PACKAGE_MANAGER GITHUB_REPO --dep=[DEPENDENCY]
#
...
OPTIONS[:package_manager] = ARGV[0]
...
fetcher = Dependabot::FileFetchers.for_package_manager(OPTIONS[:package_manager]).
...
def cache_read(name)
  ...
  repo_path = OPTIONS[:repo_name].split("/").join("@")
  cache_path = File.join("tmp", repo_path, "cache", "#{name}.bin")
...
files = cache_read("files") { fetcher.files }

# Dump dependency files in tmp/githublogin@repo-name/dependency-files
files.map do |f|
  repo_path = OPTIONS[:repo_name].split("/").join("@")
  files_path = File.join("tmp", repo_path, "dependency-files", f.name)
...
parser = Dependabot::FileParsers.for_package_manager(OPTIONS[:package_manager]).new(
  dependency_files: files,
  source: source,
  credentials: OPTIONS[:credentials]
)

dependencies = cache_read("dependencies") { parser.parse }
...
dependencies.each do |dep|
  ...
  checker = Dependabot::UpdateCheckers.for_package_manager(OPTIONS[:package_manager]).new(
    dependency: dep,
    dependency_files: files,
    credentials: OPTIONS[:credentials]
  )
  ...
  updated_deps = cache_read("updates") { get_updated_deps(checker) }
  ...
  updater = Dependabot::FileUpdaters.for_package_manager(OPTIONS[:package_manager]).new(
